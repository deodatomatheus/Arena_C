...
void MyClientMoved(int id, mov_msg mov)
{
    ...
    else if(clients[id].fight==1 && clients[id].turn) 
    {
        ...
        if(mov.msg == 'a') //se a, run
        {
            
        }
    
        if(mov.msg == 's') //se s, habilidade 2
        {
...
{
    int overHeal=0;
    overHeal=clients[id].vida/10;
    
    if(clients[id].vida <= (clients[id].vida + overHeal))
...
void baseAttack(int id)
{
    ...
    int dano=0,modifier=0;                                 //para calculo do dano causado com o ataque base 
    ...
    dano = (clients[id].ataque - clients[OID].defesa) + modifier;   //calcula o dano -> DANO = (ATK_P1 - DEF_P2) + modifier
...
void baseAttackM(int id)
{
    ...
    modifier = -10 + (rand() % 21);                      //calcula o modificador de dano (que pode ser de -10 a +10)
    dano = (clients[id].ataque - mobs[OID].defesa) + modifier;   //calcula o dano -> DANO = (ATK_P1 - DEF_P2) + modifier
...
void monsterAttack(int jog_id, int mon_id)
{
    ...
    modifier = -10 + (rand() % 21);                      //calcula o modificador de dano (que pode ser de -10 a +10)
    dano = (mobs[mon_id].ataque - clients[jog_id].defesa) + modifier;   //calcula o dano -> DANO = (ATK_P1 - DEF_P2) + modifier
...
int isMonster(int x, int y)
{
    int monster=0;
    //int id;
    if(cacheMAP[x][y]>= 97 && cacheMAP[x][y]<= 112 )
    {        
        //id=cacheMAP[x][y]-48;
        monster=1;
    }
